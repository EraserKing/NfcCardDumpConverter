...
namespace NfcCardDumpConverter.Models
{
    ...
    {
        protected byte[] RawData = new byte[1024];
        ...
        public void WriteAsRawFormat(string filePath)
        {
            ...
            Console.WriteLine($"NFC Card (Raw format) has been written to {filePath}");
        }

        public abstract void Write(string filePath);

        protected static byte FromDoubleCharToByte(string text)
        {
            if (text.Length != 2)
            {
        ...
        protected static string FromByteToDoubleChar(byte data) => $"{FromSingleByteToHexChar(checked((byte)(data / 16)))}{FromSingleByteToHexChar(checked((byte)(data % 16)))}";


        protected static byte FromSingleHexToByte(char text) => text switch
        {
        ...
        protected static char FromSingleByteToHexChar(byte data) => data switch
        {
